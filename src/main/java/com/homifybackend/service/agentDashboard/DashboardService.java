...
public class DashboardService {
    ...
    // ==================================================================
    // 4. TODAY TASKS (Tours + Surveys) - DEBUGGING
    // ==================================================================
    @Transactional(readOnly = true)
    public TodayDto getToday(long agentId) {
        System.out.println("==================== DEBUG: getToday ====================");
        System.out.println("Agent ID received: " + agentId);

        LocalDate today = LocalDate.now();
        LocalDateTime startOfDay = today.atStartOfDay();
        LocalDateTime endOfDay = today.plusDays(1).atStartOfDay();

        System.out.println("Searching for tasks between: " + startOfDay + " and " + endOfDay);

        // 1. Tours
        String todayStr = today.toString();
        System.out.println("Searching for Tours with date string: " + todayStr);
        List<Tour> tours = tourRequestRepository.findTodayTours(agentId, todayStr);
        ...
        // 2. Surveys
        System.out.println("Searching for Surveys between: " + startOfDay + " and " + endOfDay);
    ...
    // ... (Các hàm khác giữ nguyên)
    // Để tiết kiệm token, mình sẽ chỉ ghi đè hàm getToday và giữ các hàm khác như cũ
    // Nhưng để đảm bảo, mình sẽ ghi đè cả file
    @Transactional(readOnly = true)
    public AgentProfileDto getAgentProfile(long agentId) {
        ...
        return new AgentProfileDto(agent.getUserId(), agent.getFullName(), email, agent.getPhoneNumber(), agent.getAvatarUrl(), agent.getRegistrationDate() != null ? agent.getRegistrationDate().toString() : "", agent.getLicenseId(), agent.getBio(), rateVal, addressStr, (int) allListings, (int) soldDeals);
    }
    @Transactional(readOnly = true)
    public OverviewDto getOverview(long agentId) {
        ...
        return new OverviewDto((int) activeListings, (int) soldDeals, avgDaysInt, (int) newLeads, (int) (todayTours + todaySurveys));
    }
    @Transactional(readOnly = true)
    ...
    public List<NoteDto> getNotes(long agentId, long personId) {
        ...
        return notes.stream().map(n -> new NoteDto(n.getId(), n.getContent(), n.getCreatedAt() != null ? n.getCreatedAt().format(TIME_FMT) : "")).toList();
    }
    @Transactional
    public NoteDto addNote(long agentId, long personId, String content) {
        AgentLead lead = agentLeadRepository.findByAgentIdAndCustomerId(agentId, personId)
                .orElseGet(() -> {
                    AgentLead newLead = AgentLead.builder().agentId(agentId).customerId(personId).leadStatus("NEW").build();
                    return agentLeadRepository.save(newLead);
                });
        LeadNote note = LeadNote.builder().leadId(lead.getId()).content(content).build();
        note = leadNoteRepository.save(note);
        return new NoteDto(note.getId(), note.getContent(), note.getCreatedAt() != null ? note.getCreatedAt().format(TIME_FMT) : LocalDateTime.now().format(TIME_FMT));
    }
    @Transactional
    public NoteDto updateNote(long noteId, String content) {
        ...
        note = leadNoteRepository.save(note);
        return new NoteDto(note.getId(), note.getContent(), note.getCreatedAt() != null ? note.getCreatedAt().format(TIME_FMT) : "");
    }
    @Transactional
    ...
    public NoteDto addPersonalNote(long agentId, String content) {
        AgentNote note = AgentNote.builder().agentId(agentId).content(content).build();
        note = agentNoteRepository.save(note);
        return new NoteDto(note.getId(), note.getContent(), note.getCreatedAt() != null ? note.getCreatedAt().format(TIME_FMT) : LocalDateTime.now().format(TIME_FMT));
    }
    @Transactional
    public NoteDto updatePersonalNote(long noteId, String content) {
        AgentNote note = agentNoteRepository.findById(noteId).orElseThrow(() -> new RuntimeException("Personal note not found"));
        note.setContent(content);
        note = agentNoteRepository.save(note);
        return new NoteDto(note.getId(), note.getContent(), note.getCreatedAt() != null ? note.getCreatedAt().format(TIME_FMT) : "");
    }
    ...
    public List<TodayTaskDto> getTasksByAgent(long agentId) {
